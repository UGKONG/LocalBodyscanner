...
switch ($task) {
	...
	case 'GetposeCheckData': // 자세측정 체크 기본 데이터 취득
		//$user_seq = getAnyParameter("user_seq",0);
		$DEVICE_SQ = getAnyParameter("DEVICE_SQ",0);
		
		$action = 'GetposeCheckData';

		//  최종 장비 데이터 취득
		$database->prepare("
			SELECT POSECHECK_SQ,POSECHECK_NM,DIRECTION_TYPE,FROM_JOINT,TO_JOINT,CENTER_JOINT,CHECK_ORDER,ISUSE
			FROM bs_posecheck where ISUSE=1
