...
switch ($task) {
	...
	case 'NewUserExist': // 사용자 검색
			$PHONE_NO = getAnyParameter("PHONE_NO","");
			$USER_NM = getAnyParameter("USER_NM",0);
	
			$action = 'NewUserExist';
			//$pwd_encrypted = getAnyParameter("pwd_encrypted",0);
	
			$database->prepare("
				SELECT USER_SQ FROM tb_user WHERE PHONE_NO = :PHONE_NO  AND USER_NM = :USER_NM
			");
	...
	case 'UserRegisterSimple': // 사용자 간이 등록 
		$USERID = getAnyParameter("USERID","");
		$USERPWD = getAnyParameter("USERPWD","");
		$PHONE_NO = getAnyParameter("PHONE_NO","");
		...
		$database->prepare("
			INSERT tb_user (USERID,PWD_ENCRYPTED,USER_NM,PHONE_NO,GENDER,CENTER_SQ, GRADE, ISUSE, LAST_DT, REG_DT)
			select :USERID, SHA2(:PWD_ENCRYPTED, 256),:USER_NM, :PHONE_NO, :GENDER, CENTER_SQ, 1, 1, now(), now() from tb_device where DEVICE_SQ=:DEVICE_SQ
		");
		$database->bind(':USERID', $USERID);
