...
switch ($task) {
	...
	case 'UserRegisterSimple': // 사용자 간이 등록 
		$USERID = getAnyParameter("USERID","");
		$PHONE_NO = getAnyParameter("PHONE_NO","");
		$GENDER = getAnyParameter("GENDER",1);
		$DEVICE_SQ = getAnyParameter("DEVICE_SQ",0);
	...
	case "SetPoseAngle": // pose Joint data insert
			...
			//$pwd_encrypted = getAnyParameter("pwd_encrypted",0);
						$database->prepare("
				INSERT bs_posedata (POSESTADARD_SQ,MEASUREMENT_SQ,DIRECTION,ANGLE,REG_DT)
				values (:POSESTADARD_SQ , :MEASUREMENT_SQ,:DIRECTION,:ANGLE,now())
			");
			...
			if ($database->rowCount() == 0) {
				// $response_array["result"] = 'fail';
				// $response_array["POSESTADARD_SQ"] = 0;
				$note = 'POSE JOINT ANGLE 실패';
			}
			else {
				// $database->prepare("
				// 	select LAST_INSERT_ID() USER_SQ;
				...
				// $response_array["result"] = 'success';
				// $response_array["POSESTADARD_SQ"] = $row["POSESTADARD_SQ"];
				$note = 'POSE JOINT ANGLE 성공';
			}
