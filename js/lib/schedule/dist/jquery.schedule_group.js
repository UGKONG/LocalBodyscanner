var SELECTED_DATE = '';
var SELECTED_TEMP_INDEX = '';
var SCHEDULESTATISTICS = [];
var TARGET_DATE = '';
var SELECTED_CLASS;
var TEMP_TIME;
var TEMP_SQ;
...
(function ($, window, document, undefined) {
  ...
  var defaults = {
    ...
    //   periodBackgroundColor: 'rgba(82, 155, 255, 0.5)',
      periodBackgroundColor: '#36925f',
      periodBorderColor: '#2a3cff',
      periodTextColor: '#fff',
      periodRemoveButton: '삭 제',
      periodDuplicateButton: ' ',
      periodTitlePlaceholder: '회원검색',
      daysList: addArrayText(dateFormat(TARGET_DATE)),
    //   daysList: [
    //     '일',
    //     '월',
    //     '화',
  ...
  $.extend(Plugin.prototype, {
    ...
    init: function () {
      ...
      if (this.settings.mode === 'edit') {
        ...
        $(this.element).on('mousedown', '.jqs-day', function (event) {
            ...
            // let week = $('.topleft button.active').index() == 0 ? true : false;
            // if(!week){   // [주]
            //     if( $('#schedule-date .jqs-grid-head > div').eq($(this).parent().index()).attr('class').indexOf('null') > -1 ){
            //         return false;
            //     }
            // }
            ...
            SELECTED_TEMP_INDEX = $(this).parent().index();
            SELECTED_DATE = $('#schedule-week .jqs-grid-head > div').eq(SELECTED_TEMP_INDEX).text();
        ...
        $(this.element).on('mouseup', '.jqs-day', function (event) {
          ...
          if (!$(event.target).hasClass('jqs-period') && $(event.target).parents('.jqs-period').length === 0 || !$(event.target).hasClass('jqs-period-remove')) {
            var offset = event.pageY - $(this).offset().top;
            var height = Math.round(offset / $this.periodPosition) - position;


            if($('section.content > div.top_sMenu > .topleft button.active').index() == 0){
                SELECTED_TEMP_INDEX = $(this).parent().index();
                SELECTED_DATE = $('#schedule-week .jqs-grid-head > div').eq(SELECTED_TEMP_INDEX).text();
            ...
            if($this.add($(this), position, height)) {
                // ADD_SOLO_CLASS();
                // $this.settings.onMouseUp.call(this, position, height, TEMP_TIME);
                TEMP_TIME = $this.periodInit(position, position + height);
                $this.settings.onMouseUp.call($this, event, $this.last_period, $this.element,TEMP_TIME);
            }
          }
    ...
    create: function () {
      ...
      $('<table class="jqs-table jswscr1"><tr></tr></table>').appendTo($(this.element));

      
      for (var i = 0; i < this.settings.days; i++) {
          if(i >= this.settings.daysList.length){
            var style = 'background-color:#00000010';
          }else{
            var style = '';
          }
        $('<td><div class="jqs-day" style="' + style + '"></div></td>').
          appendTo($('.jqs-table tr', this.element));
      }

      $('<div class="jqs-grid"><div class="jqs-grid-head jswscr2"></div></div>').appendTo($(this.element));
      ...
      for (var k = 0; k < this.settings.days; k++) {
        let temp = this.settings.daysList[k];
        $('<div class="jqs-grid-day ' + (temp == "undefined" ? 'null' : '') + '">' + (temp == "undefined" ? '' : temp) + '</div>').appendTo($('.jqs-grid-head', this.element));
      }
    ...
    add: function (parent, position, height, options) {
      ...
      var period = $('<div class="jqs-period">' +
        ...
        'top': position * this.periodPosition,
        'height': height * this.periodPosition
      }).attr('id', this.uniqId()).attr('title', '수정 : 클릭').appendTo(parent);

      $('.jqs-period-remove').click(function(){
      ...
      this.last_period = period;
    //   this.selected_period_time = this.periodInit(position, position + height);
      ...
      if (this.settings.mode === 'edit') {
        ...
        if (this.settings.periodOptions) {
            period.not($('.jqs-period-remove')).mousedown(function (event) {
                var x = event.pageX,
                    y = event.pageY;

                if(event.button == 0){
                    SELECTED_CLASS = $(this).find('.detail_info').attr('data-seq');
                    if(!$(event.target).hasClass('jqs-period-remove') || !$(event.target).hasClass('jqs-options-cancel') || !$(event.target).hasClass('jqs-options-close')) {
                        $this.settings.onClickPeriod.call(this, event, this, $this.element);
                    ...
                    if($('button#solo_s_view_button').attr('class') == 'active'){
                        SELECTED_CLASS = this.firstElementChild.lastElementChild.getElementsByClassName('detail_info')[0].attributes[1].value;
                        
                        rightClick1(x,y);      // 우클릭 이벤트 추가
                    }else{
                        SELECTED_CLASS = this.firstElementChild.lastElementChild.getElementsByClassName('detail_info')[0].attributes[1].value;
                        rightClick2(x,y);      // 우클릭 이벤트 추가
                    }
                  }
            });
        }
    }
    // 우클릭 시 메뉴 위치 설정 함수 정의
    function rightClick1(x,y){
        let rClick = $('#rightClick_div1');
        var screenW = ($(document).width() / 3) * 2,
            screenH = ($(document).height() / 3) * 2 ;
        var divW = rClick.outerWidth(),
            divH = rClick.outerHeight();
      ...
      // 우클릭 시 메뉴 위치 설정 함수 정의
    function rightClick2(x,y){
      let rClick = $('#rightClick_div2');
      var screenW = ($(document).width() / 3) * 2,
          screenH = ($(document).height() / 3) * 2 ;
      var divW = rClick.outerWidth(),
          divH = rClick.outerHeight();
    ...
    openOptions: function (event, period) {
      ...
      $('#time_period').text(this.periodInit(position, position + height));
      $('.time_period').text(this.periodInit(position, position + height));
      ...
      $('#solo_searchName', this.element).change(function () {
      ...
      $('.jqs-options-remove').click(function(){
        $this.remove(period);
        $this.closeOptions();
    ...
    periodText: function (period) {
      ...
      $('.jqs-period-title', period).css('min-height','20px');
    },
